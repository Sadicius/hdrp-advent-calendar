...
lib.callback('hdrp-advent-calendar:client:getDate', false, function(data)
        ...
        if dateData then
            if (data.year == dateData.date.year and data.month == dateData.date.month and data.day >= dateData.date.day) then
                Options[i] = {
                    ...
                    onSelect = function()
                        TriggerServerEvent("hdrp-advent-calendar:server:giveitems", sortedDates[i], dateData.date)
                    end,
                }
            end
        else
...
lib.callback.register('hdrp-advent-calendar:server:progress', function()
...
RegisterCommand('calendar-advent', calendarOpen)
RegisterNetEvent('hdrp-advent-calendar:client:open', calendarOpen)
